...
class LeafPattern(Pattern):
    ...
    def match(
        ...
        if same_name and type(self.value) == list:
            ...
            if (
                same_name[0].value is not None
                and increment is not None
                and isinstance(same_name[0].value, type(increment))
            ):
...
def docopt(
    ...
    if matched and left == []:
        ...
        if (
            more_magic
            and target_parent_frame
            and not output_value_assigned
            and not target_parent_frame.f_globals.get("arguments")
        ):
